"""Shared test fixtures. Unit/contract/integration tests run with no network.

`demo_fin` / `make_demo_financials` replicate the seed prototype's
SyntheticSource figures exactly (seed/data.py), so results are directly
comparable to the validated seed engine's output (golden numbers in
tests/unit/domain/test_dcf.py).
"""
...
@pytest.fixture
def demo_factory() -> Callable[..., CompanyFinancials]:
    """The factory itself, for tests needing several distinct companies."""
    return make_demo_financials
